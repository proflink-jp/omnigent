...
from omnigent._wrapper_labels import OMP_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
from omnigent.omp_native_bridge import bridge_dir_for_session_id

_logger = logging.getLogger(__name__)

_DEFAULT_PI_COMMAND = "omp"
    ...
    *,
    ...
    if server is None:
        raise click.ClickException(
            ...
            "_ensure_backend before run_omp_native."
        )
    with TemporaryDirectory(prefix="omnigent-omp-native-") as tmpdir:
...
def _materialize_pi_agent_spec(tmpdir: Path) -> Path:
    ...
    """
    yaml_path = tmpdir / "omp-native-ui.yaml"
    raw: dict[str, Any] = {
        "name": _AGENT_NAME,
        "prompt": (
    ...
    async with httpx.AsyncClient(base_url=base_url, headers=headers, timeout=timeout) as client:
        ...
        if session_id is None:
            ...
            if (
                ...
                raise click.ClickException(
                    f"Conversation {session_id!r} is not a omp-native session."
                )
...
async def _create_pi_session(
    ...
    *,
    ...
    resp = await client.post(
        ...
        files={"bundle": ("omp-native-ui.tar.gz", bundle, "application/gzip")},
        timeout=120.0,
    )
    if resp.status_code >= 400:
        raise click.ClickException(
