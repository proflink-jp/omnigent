// Unit test for the omp-native bridge extension's interrupt / replay logic.
//
// Regression coverage for F18 (SDK_INTEGRATION_BUG_AUDIT.md): an interrupt that
// arrives while Omp is idle used to arm a 30s replay window that aborted the next
...
//
// Run with: node omnigent/resources/omp_native/omnigent_omp_native_extension.test.js
//
// Manual reproduction of the original bug (for context):
//   1. Start a native Omp session linked to Omnigent and let it go idle.
...
const EXT_PATH = path.resolve(__dirname, "omnigent_omp_native_extension.js");
...
function makeHarness({ captureEvents = false } = {}) {
  const inboxDir = fs.mkdtempSync(path.join(os.tmpdir(), "omp-native-inbox-"));
