// Auto-generated Omnigent bridge extension for native Omp sessions.
const fs = require("fs");
...
// re-mints it into config.json each turn (OmpNativeExecutor), so re-reading
...
/**
 * Evaluate a TOOL_CALL policy for a native Omp tool via the Omnigent server's
 ...
 *   - POLICY_ACTION_DENY  → block the Omp tool call with the policy reason.
 ...
 * omnigent.policies.types.FAIL_CLOSED_PHASES and the Python native hook's
 * fail_closed_hook_output(PreToolUse) → deny. (omp-native is itself a sole
...
/**
 * Build a Omp tool-result object from an MCP ``tools/call`` JSON-RPC response.
 *
 * Omp expects ``{ content: [{ type: "text", text }], isError }``. The Omnigent
 ...
 * tool errors) on failure. Map both into a single text block so the Omp agent
 ...
 * and return the parsed response (or a fail-closed Omp tool-result on a
 ...
 * (``isError: true``) rather than throwing, so a server hiccup never wedges Omp's
 * agent loop.
 */
async function callOmnigentTool(config, toolName, args) {
  ...
  if (initial === null) {
    // ALLOW / DENY / executed — map directly to a Omp tool result.
...
/**
 * Lift token usage out of one Omp assistant message.
 *
 * Mirrors the non-native executor's ``_extract_pi_turn_usage``
 * (omnigent/inner/pi_executor.py): Omp (``@earendil-works/pi-coding-agent``)
 * carries a per-message ``usage`` object with ``input`` / ``output`` /
 * ``cacheRead`` / ``cacheWrite`` / ``totalTokens`` counts, and the message
 * carries the resolved ``model``. Omp's ``input`` is the NON-cached input
 ...
 */
function extractOmpUsage(message) {
  ...
  // No countable tokens means Omp emitted an empty usage object — treat as "no
...
async function postEvent(config, body) {
  ...
  try {
    ...
    // Keep Omp responsive even if Omnigent is temporarily unavailable.
  }
}
...
/**
 * Trigger Omp's own context compaction on the resident ExtensionContext.
 *
 * Omp owns its context window inside this TUI process, so explicit /compact
 ...
 * fire-and-forget (returns void); Omp summarises older messages and appends a
 ...
 * "Compacting conversation…" spinner tracks Omp's real progress.
 *
 ...
 *     Omp's onComplete/onError, return true.
 */
    ...
    await postEvent(config, {
      ...
      data: {
        ...
        item_data: {
          ...
          message:
            "Omnigent: /compact is unavailable for this Omp session. The " +
            "resident Omp context exposes no compaction API, so the model or " +
            "Omp version may not support it.",
        },
      },
    });
    return false;
...
/**
 * Apply a web-picked model switch to the resident Omp process.
 *
 * Omp owns the active model inside this TUI process, so a model picked in the
 ...
 * same catalog Omp's own ``/model`` picker uses, sourced from the generated
 * models.json) and calls Omp's ``setModel`` — immediate, no ``/reload``.
 *
...
async function applyModelChange(pi, config, ctx, modelId) {
  ...
  // would fail every switch on a hypothetical Omp build exposing only
  ...
  if (!pi || typeof pi.setModel !== "function" || !listModels) {
    await postModelChangeError(
      config,
      `Omnigent: could not switch to model "${id}" — this Omp session exposes ` +
        "no model-switch API (the model or Omp version may not support it).",
    );
    return false;
  }
  let model;
  ...
  if (!model) {
    await postModelChangeError(
      config,
      `Omnigent: model "${id}" is not available in this Omp session.`,
    );
    return false;
  }
  try {
    ...
    await postModelChangeError(
      config,
      `Omnigent: switching to model "${id}" failed inside Omp.`,
    );
    return false;
  }
}
...
/**
 * Report Omp's live model catalog to Omnigent for the Web UI model picker.
 *
 * Sourced from Omp's model registry — the models Omp actually loaded for THIS
 * session, whatever their origin: an Omnigent-configured provider's generated
 * models.json, or Omp's own ``/login`` / ``~/.pi`` config. Posting the live
 ...
 * ``getAll()`` (Omp's entire built-in catalog spanning every vendor, most with
 ...
 * (older Omp). Best-effort and fire-and-forget: an empty or unavailable
            ...
            data: {
              ...
              item_data: {
                ...
                message:
                  ...
                  `not be delivered to Omp after ${MAX_DELIVER_ATTEMPTS} attempts ` +
...
module.exports = function (pi) {
  ...
  // Live streaming state for assistant text deltas. Omp emits
  ...
  // policies, double-prompt. Omp's OWN built-in tools (read/shell/etc) are not
  ...
  if (config && Array.isArray(config.tools)) {
    for (const tool of config.tools) {
      ...
      // Omp passes tool.parameters straight to the LLM as JSON Schema, so the
      // Omnigent schema is usable as-is. execute() round-trips the call to the
      // Omnigent server's MCP proxy and returns the result to Omp.
  ...
  // Cumulative session token usage. Omp reports PER-MESSAGE counts (one
  ...
  // arriving on multiple lifecycle events is only counted once. Omp's
  ...
  function accumulateUsage(message) {
    const usage = extractOmpUsage(message);
  ...
  // no-op. Fail-open via ``postEvent`` so a failed POST never wedges Omp.
  ...
  function requestInterrupt(ctx) {
    // ctx.abort() is a silent no-op when the Omp agent is idle (it does NOT
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    // Publish Omp's live model catalog so the Web UI picker populates from what
    // Omp actually loaded, independent of how it authenticated.
    await postModelOptions(config, ctx);
    // Report the model Omp launched with so the composer pill and the picker's
  ...
  pi.on("model_select", async (event, ctx) => {
    rememberContext(ctx);
    // Mirror a model switch made inside the Omp TUI (the ``/model`` command or
    // Ctrl+P cycling) back to Omnigent so the web picker reflects it. Skip
    // ``restore`` — that is Omp re-applying the session's saved model at
  ...
  pi.on("agent_start", async (_event, ctx) => {
    ...
    // Ompn the response_id for this agent loop. agent_end MUST emit the same id
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    // ASK policies, a double prompt. Omp's own built-in tools (read/shell/etc)
    ...
    // happens immediately for omp-native — just enqueue + TurnComplete), so
  ...
  pi.on("message_end", async (event, ctx) => {
    ...
    // posted and this finalize agree on the id regardless of whether Omp
    // fires message_start.
  ...
  pi.on("turn_end", async (event, ctx) => {
    ...
    // Fallback usage capture: if Omp attached usage to the turn's final
