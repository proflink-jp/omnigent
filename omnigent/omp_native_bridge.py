...
OMP_NATIVE_BRIDGE_DIR_ENV_VAR = "HARNESS_OMP_NATIVE_BRIDGE_DIR"
OMP_NATIVE_REQUEST_SESSION_ID_ENV_VAR = "HARNESS_OMP_NATIVE_REQUEST_SESSION_ID"
OMP_NATIVE_CONFIG_ENV_VAR = "OMNIGENT_OMP_NATIVE_CONFIG"

_BRIDGE_ROOT = Path.home() / ".omnigent" / "omp-native"
_CONFIG_FILE = "config.json"
_EXTENSION_FILE = "omnigent_omp_native_extension.js"
_EXTENSION_PACKAGE = "omnigent.resources.omp_native"
_INBOX_DIR = "inbox"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    ...
    :returns: Absolute bridge directory under ``~/.omnigent/omp-native``.
    """
...
def build_omp_native_spawn_env(conversation_id: str) -> dict[str, str]:
    """
    Build spawn env for the ``omp-native`` harness process.

    :param conversation_id: Omnigent conversation id.
    :returns: Environment variables needed by the Pi-native harness executor.
    """
    return {
        OMP_NATIVE_BRIDGE_DIR_ENV_VAR: str(bridge_dir_for_session_id(conversation_id)),
        OMP_NATIVE_REQUEST_SESSION_ID_ENV_VAR: conversation_id,
    }
