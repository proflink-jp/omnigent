...
# base_url; omp-native rewrites it to the Anthropic surface Pi speaks natively.
...
def _databricks_pi_provider(entry: ProviderEntry, *, model: str | None) -> PiProviderConfig | None:
    ...
    try:
        ...
        _LOGGER.info(
            "omp-native: falling back to single-model display (could not resolve credentials)"
        )
        claude_models = []
        gpt_models = []
        completions_models = []
...
def _fetch_pi_model_lists(
    ...
    * Other LLMs (Kimi, Llama, GLM, Gemini, older GPT …) that work with
    ...
    try:
        ...
        _LOGGER.warning(
            "omp-native: could not fetch Databricks model list; "
    ...
    if not claude and not gpt_responses and not completions:
        _LOGGER.info(
            "omp-native: Databricks serving-endpoints returned no LLM models; "
            "Pi will show only the selected model"
        )
...
def _cli_config_databricks_transport(entry: ProviderEntry) -> CodexConfigTransport | None:
    ...
    if transport is None:
        ...
        for alt_config in sorted(codex_dir.glob("config*.toml")):
            ...
            if transport is not None:
                _LOGGER.info(
                    "omp-native: cli-config provider %r (model_provider %r) found in %s",
                    entry.name,
    ...
    if transport is None:
        _LOGGER.info(
            "omp-native: cli-config provider %r (model_provider %r) has no resolvable "
    ...
    if not _is_databricks_ai_gateway_url(transport.base_url):
        _LOGGER.info(
            "omp-native: cli-config provider %r (model_provider %r, base_url %r) is not a "
    ...
    if not transport.auth_command:
        ...
        try:
            ...
            _LOGGER.info(
                "omp-native: cli-config provider %r has no auth command; "
            ...
            _LOGGER.info(
                "omp-native: Databricks cli-config provider %r (model_provider %r) "
...
def _cli_config_pi_provider(entry: ProviderEntry, *, model: str | None) -> PiProviderConfig | None:
    ...
    ``cli-config`` provider. Codex-native routes through that table; omp-native
    ...
    if _DATABRICKS_AI_GATEWAY_LABEL in gateway_labels:
        ...
        try:
            ...
            _LOGGER.info(
                "omp-native: cli-config path could not resolve workspace URL "
    ...
    if real_workspace_url and transport.auth_command:
        ...
        if token:
            ...
            _LOGGER.info(
                "omp-native: auth command produced no token; Pi will show only the selected model"
            )
...
def resolve_omp_native_provider(
    *,
    model: str | None = None,
    ...
    try:
        ...
        if entry is None:
            _LOGGER.info(
                "omp-native: no omnigent-configured provider for the pi/anthropic/openai "
        ...
        if entry.kind == DATABRICKS_KIND:
            ...
            _LOGGER.info(
                "omp-native: configured provider %r (kind %r) cannot drive Pi; "
        ...
        if resolved is None:
            ...
            _LOGGER.warning(
                "omp-native: configured provider %r (kind %r) could not be translated "
            ...
            if resolved is None:
                _LOGGER.warning("omp-native: no usable provider found; Pi will use its own login.")
        return resolved
        ...
        _LOGGER.warning(
            "omp-native: failed to resolve the omnigent-configured provider; Pi will "
            "use its own login.",
            exc_info=True,
        )
        return None
...
def omp_native_provider_launch(
